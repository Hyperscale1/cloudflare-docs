...
**[Network policies](/cloudflare-one/traffic-policies/network-policies/)** inspect individual TCP, UDP, and GRE (Generic Routing Encapsulation) packets. They can match on IP addresses, ports, protocols, and the server name sent at the start of an encrypted connection (Server Name Indication, or SNI).
...
**[Egress policies](/cloudflare-one/traffic-policies/egress-policies/)** control how traffic leaves your network. By default, traffic proxied through Gateway exits from shared Cloudflare IP addresses. Egress policies let you assign fixed IP addresses that belong to your organization, so third-party services can recognize the traffic as yours.
