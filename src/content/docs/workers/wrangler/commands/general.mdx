...
- `--secrets-file` <Type text="string" /> <MetaInfo text="optional" />
  - Path to a file containing secrets to upload alongside the deployment. Accepts JSON or `.env` format — the same formats used by [`wrangler secret bulk`](/workers/wrangler/commands/#secret-bulk). Existing secrets not included in the file are preserved from the previous version. Refer to [Secrets — Upload secrets alongside code](/workers/configuration/secrets/#upload-secrets-alongside-code) for more details.
