...
You can upload secrets at the same time as your Worker code using the `--secrets-file` flag on [`wrangler deploy`](/workers/wrangler/commands/#deploy) or [`wrangler versions upload`](/workers/wrangler/commands/#versions-upload). This accepts a path to a JSON or `.env` file — the same formats accepted by [`wrangler secret bulk`](/workers/wrangler/commands/#secret-bulk).
